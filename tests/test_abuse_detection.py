...
def test_stock_data_types(stock_df):
    ...
    #assert isinstance(stock_df['High'], float)
...
@dt.working_directory(__file__)
def traders_df():
    return pd.read_csv('../traders_data.csv')
...
def test_traders_data_types(traders_df):
    ...
    dt.validate(traders_df['volume'], int)
    #dt.validate(traders_df['tradeDatetime'], DatetimeIndex)
 
def test_traders_tradeId(traders_df):
